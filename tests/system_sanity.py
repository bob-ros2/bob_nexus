...
class NexusIntegrationTests(unittest.TestCase):
    ...
    def test_02_cli_infrastructure(self):
        ...
        self.assertEqual(
            res.returncode, 0, msg=f"Spawn mastermind failed. Out: {res.stdout} Err: {res.stderr}"
        )
        self.assertTrue(
            os.path.exists("entities/master/test_mastermind/llm.yaml"),
            msg="llm.yaml missing after spawn",
        )
        ...
        self.assertEqual(
            res.returncode, 0, msg=f"Spawn assistant failed. Out: {res.stdout} Err: {res.stderr}"
        )
        self.assertTrue(
            os.path.exists("entities/assistant/test_alice/llm.yaml"),
            msg="llm.yaml missing after spawn",
        )
    ...
    def test_03_oai_chat_and_tools(self):
        ...
        self.assertIn(
            "NEXUS CALLING: memory__store_memory",
            stdout,
            msg=f"Tool call log missing. Out: {stdout}",
        )
        # Verify the final response after tool execution
        self.assertIn(
            "successfully saved your poem", stdout, msg=f"Success response missing. Out: {stdout}"
        )
