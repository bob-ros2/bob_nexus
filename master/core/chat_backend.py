...
class OAIBackend(ChatBackend):
    def __init__(
        ...
        **kwargs
        ...
        if self.persistent_path: self._load_persistent_history()

    def _load_persistent_history(self):
        if not os.path.exists(self.persistent_path): return
        try:
            ...
            for b in blocks:
                ...
                in_content = False
                for l in lines:
                    if l.startswith("ROLE: "): role = l[6:].strip()
                    elif l.startswith("CONTENT:"): in_content = True
                    elif in_content: content.append(l)
                if role and content:
        ...
        except Exception as e: print(f"Warning: History load error: {e}")

    def _save_to_persistence(self, role, content, tool_calls=None):
        if not self.persistent_path: return
        ...
        try:
            ...
            with open(self.persistent_path, "a") as f:
                f.write("---\nROLE: " + role + "\nTIMESTAMP: " + t + "\n")
                if tool_calls: f.write("TOOL_CALLS: " + json.dumps(tool_calls) + "\n")
                f.write("CONTENT:\n" + content + "\n")
        except Exception as e: print(f"Warning: History save error: {e}")
    ...
    def send_message(self, message: str, stream: bool = True):
        ...
        while loop < self.max_tool_calls:
            ...
            try:
                ...
                self._save_to_persistence("assistant", msg.get("content", "") or "", tool_calls=tool_calls)
...
class BobLLMBackend(ChatBackend):
    def __init__(self, input_topic, output_topic):
        if not rclpy: raise ImportError("rclpy not found")
    ...
    def _cb(self, msg):
        if msg.data == "[DONE]": self.finished = True
        else: self.chunk_queue.append(msg.data)
    ...
    def send_message(self, message: str, stream: bool = True):
        ...
        while not self.finished and (time.time() - start < 30):
            ...
            while self.chunk_queue: yield self.chunk_queue.pop(0)
        self.node.destroy_subscription(sub)

def get_backend(backend_type, **kwargs):
    if backend_type == "oai": return OAIBackend(**kwargs)
    if backend_type == "bob_llm": 
        return BobLLMBackend(kwargs.get("topic_in", "llm_prompt"), kwargs.get("topic_out", "llm_stream"))
